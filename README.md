...
`python examples/demo.py` opens a playground.
...
examples/demo.py   tools/gallery.py   .github/workflows/ci.yml
```
...
> **Verification status.** The core, framework and tool state-machine tests pass, and the
> wheel builds and installs. The authoring sandbox had no Qt binding, so the Qt layer has so
> far only run against the stub in `tests/_qt_stub.py`. The first CI run (or
> `python examples/demo.py`) is the real check of the binding-specific enum handling.
